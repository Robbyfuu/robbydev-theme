...
### Added
- New Neon Dreams glow screenshot in README to showcase the per-token halo with crisp text body.

## [2.0.3] — 2026-04-27

### Fixed
- Glow no longer smudges the text body. Removed the inner `2px` text-shadow layer that was painting a same-color halo right against each glyph, blurring the outline. Single outer halo at `6px` keeps the text crisp while still emitting a clear neon glow per token color. Reference: SynthWave '84 uses the same single-layer technique.

## [2.0.2] — 2026-04-27

### Fixed
- "Your installation appears to be corrupt" warning is now silenced automatically. After patching `workbench.html`, the extension recalculates its SHA256 and updates `product.json` so the integrity check passes. Disable also restores the original checksum.
